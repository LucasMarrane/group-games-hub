...
import { AdaptedTheme, ThemeExpansion } from '@appTypes/ito';
import { Button } from '@shadcn/components/ui/button';
import { ItoThemes } from '@data/index';

const adaptThemes = (data: ThemeExpansion): AdaptedTheme[] => {
    return data.items.map((item) => {
        const scale = new Map<number, string>(item.scale);
        const category = data.categories.find((c) => c.id === item.category);
        const scaleMin = scale.get(1) ?? 'Minímo';
        const scaleMax = scale.get(100) ?? 'Maxímo';
        ...
        return {
            ...
            scaleMin,
            scaleMax,
            sourcePack: data.name,
        };
    });
};

export const SincroniaGame = () => {
    const themes = useMemo(() => adaptThemes(ItoThemes[0] as ThemeExpansion), []);
    ...
    return (
        <div className='min-h-full p-4 flex flex-col gap-6'>
            ...
            <div className='text-center'>
                <h1 className='text-3xl font-display font-bold text-gradient-sincronia mb-2'>ITO (Sincronia)</h1>
                    ...
                    {currentTheme ? (
                        <motion.div key={currentTheme.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className='space-y-4'>
                            ...
                            <div className='flex items-center gap-4 pt-4 border-t border-border'>
                                ...
                                <div className='flex-1 h-2 rounded-full gradient-sincronia' />
                                <div className='flex-1 text-center'>
                                    <div className='text-2xl font-bold text-sincronia-glow'>100</div>
